...
<div class="card fetch-card">
    <h3 style="font-family:'Fraunces',serif;margin-bottom:10px;">URL Sağlık Monitörü</h3>
    <p style="font-size:14px;color:var(--text-muted);margin-bottom:15px;">Bu araç dahili ağdaki servislerin durumunu kontrol etmek için tasarlanmıştır. Hiçbir filtreleme uygulanmaz.</p>
    ...
    <div style="font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--text-muted);background:var(--bg);padding:10px;border-radius:6px;margin-bottom:15px;">
        GET /fetch.php?url=&lt;URL&gt; — Filtre: Devre Dışı
    </div>

    <div class="form-row">
        <input id="url-input" class="form-input" type="text" placeholder="URL giriniz" autocomplete="off">
        <button class="btn" id="fetch-btn">İsteği Gönder</button>
    </div>
...
<div class="card">
    ...
    <p>Hedefiniz, sunucunun sadece dahili erişime izin verdiği <strong>http://127.0.0.1/secret.php</strong> adresine erişerek gizli bayrağı ele geçirmektir.</p>
</div>
...
(function(){
    ...
    async function doFetch(){
        ...
        body.textContent='İstek gönderiliyor...';btn.disabled=true;
        try{
